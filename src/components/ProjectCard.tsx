...
const getStatusBadgeClassesLight = (status: string) => {
  const s = status.trim().toLowerCase();
...
const ProjectCard = ({
  ...
  const badgeClasses =
    statusColor ??
    (isDark ? getStatusBadgeClasses(status) : getStatusBadgeClassesLight(status));
        ...
        {techStack.map((t) => (
          <span
            ...
            className={`rounded-md px-2.5 py-1 text-[11px] font-medium tracking-wide ${
              isDark
                ? "bg-white/[0.07] text-zinc-300 ring-1 ring-white/[0.08]"
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            {codeUrl && codeUrl !== "#" && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold ring-1 transition-all duration-200 ${
                  isDark
                    ? "ring-white/[0.12] text-zinc-300 hover:bg-white/[0.08] hover:text-white"
                    : "ring-zinc-200 text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900"
                }`}
              >
